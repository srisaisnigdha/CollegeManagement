...
const BonafideCertificate = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/getCertificates/');
  ...
  const handleResolveMessage = async (index) => {
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/acceptCertificate/', formData);
      setChanges(change + 1);
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="bonafide-requests-list">
        <div className="header">
          <p>Bonafide Certificates Requests</p>
        </div>
        {/* <ul>
